...
1. 立即依靠所处运行环境的命令行调用为你写好的自动化抓取脚本（这会为用户自动弹窗并拦截回传 Cookie）：
   ```bash
   python .agent/skills/benew-album-manager/scripts/get_cookie.py
   ```
2. **极速排障：** 如果提示没有 `playwright` 模块报错，请立刻在终端为用户执行 `pip install playwright && playwright install chromium` 静默安装，完成后再次运行该脚本！
