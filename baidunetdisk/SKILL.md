...
用于 AI Agent 的百度网盘操作 Skill，支持完整的文件管理功能：文件列表查看、搜索、分享链接提取、转存、目录创建、文件增删改查等。
...
作为 AI Assistant，当你被要求执行百度网盘相关操作时，你需要 `BDUSS` 和 `STOKEN` 凭证。

**如果当前未配置凭证或凭证已失效，请绝对不要询问用户提供！** 你需要自主完成排障：

1. 请立即在终端中执行内置的 CDP 抓取脚本，它会自动调起浏览器的调试模式来获取：
   ```bash
   python "你的绝对路径/.../scripts/get_cookie_cdp.py"
   ```
2. 脚本执行成功后，它会自动将最新的凭证写回本技能目录下的 `config.json`。请你再次读取该 JSON 文件，或者直接通过后续的 `main.py` 调用（`main.py` 内部也会读取它）来继续执行用户最初委托给你的网盘操作。
...
如果你在本地运行 Agent 程序，并且装有 Chrome 浏览器，可以直接对 Agent 说：
`"登录百度网盘"` 或 `"获取百度网盘凭证"`
Skill 会自动打开带有调试端口的 Chrome，并在您登录后抓取凭证并自动填入配置。
...
# 浏览分享链接的指定子目录（注：path 需使用在 extract 中返回的绝对路径）
python scripts/main.py list_share share_url=https://pan.baidu.com/s/1xxxxx extract_code=abcd path="/我的资源/分享文件/子目录"
```
...
# 转存分享链接
"把这个百度网盘分享链接转存到我的网盘: https://pan.baidu.com/s/1xxxxx 提取码: abcd"

# 创建目录
"在百度网盘创建一个名为'工作文档'的目录"

# 重命名文件
"把百度网盘里的'旧文件名.txt'重命名为'新文件名.txt'"

# 删除文件（谨慎）
"删除百度网盘里的'/测试/临时文件.txt'"
```
